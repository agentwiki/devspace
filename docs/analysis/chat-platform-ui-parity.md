...
Slack이 1차 표면: 세션 스레드, 승인 버튼, 상태 chat.update, 스트림 코얼레싱,
App Home 세션 목록(실데이터), set-secrets 모달, 리포 픽커 모달까지 구현.
Discord는 M7에서 UI 동급 도달: 코어 루프(스레드 세션, 버튼, 상태 edit,
스트림 edit — M6-C, Forum 대신 일반 채널+스레드로 구현)에 더해 set-secrets
모달·리포 픽커 모달(M7-B, custom_id가 스레드 컨텍스트를 운반)과 `/sessions`
세션 목록(M7-C, App Home과 동일한 listSessions 읽기 → 에페메랄 응답)까지
구현. Forum 글1=세션 대시보드는 표현 업그레이드로 M8+ 이연. 스트림은
Discord 2000자 한도 때문에 꼬리(tail) 윈도우만 표시.
