...
## 13. 시나리오 잡도 갱신된 codex 인증을 캐시에 되저장한다

**결정: `scenarios` 잡 끝에 `codex-auth.sh save` + `actions/cache/save`를 `if: always()`로 추가해, 이 run에서 codex가 회전시킨 auth.json을 다음 run을 위해 보존한다. (이전엔 주간 keepalive 잡만 저장했다.)**

- **왜(관측된 실패):** codex 구독 인증의 refresh token은 **1회용(회전식)**이다 —
  한 번 갱신하면 서버가 그 토큰을 폐기하고 새 토큰을 발급한다(§2). 그런데
  `scenarios` 잡은 캐시를 **복원만** 하고 저장하지 않아서, 첫 run이 토큰을
  회전시키면 그 새 토큰이 러너와 함께 사라지고, **다음 run은 이미 소비된 토큰을
  복원**해 `refresh_token_reused`(401)로 codex가 전면 실패했다(실제 run 28806525217:
  codex 의존 4개 spec 전패, 순수 UI 1개만 통과). §11로 run 빈도·codex 세션이
  늘며 이 잠재 버그가 드러났다.
- **왜 always인가:** codex가 토큰을 회전시킨 뒤 테스트가 다른 이유로 실패해도,
  회전된 새 토큰을 저장하지 않으면 다음 run이 소비된 토큰을 재사용해 깨진다.
  그래서 성패와 무관하게 **마지막으로 회전된 auth.json**을 보존한다. 인증이 아예
  실패한 run은 auth.json이 (복원한) 그대로라, 되저장해도 상태가 나빠지지 않는다.
- **한 run 안 순차 갱신은 이미 안전:** `~/.codex`가 샌드박스에 바인드 마운트라
  (§9) 순차(workers:1, playwright.config.ts)로 도는 codex 세션들이 같은 auth.json을
  차례로 갱신·되쓰기 한다. 그래서 문제는 run 사이(cross-run) 보존뿐이었다.
- **남은 위험(동시 run):** 두 워크플로 run이 겹치면 같은 토큰을 각자 회전시키려
  경쟁해 한쪽이 `refresh_token_reused`를 맞을 수 있다. 단일 사용자라 드물어 지금은
  받아들이고, 잦아지면 잡/워크플로 `concurrency` 그룹으로 직렬화한다.
- **시크릿 되저장은 안 쓴다:** 갱신본을 secret 원본에 쓰는 것도 가능하나(REST +
  sodium 봉인), `secrets: write` admin PAT를 CI에 상주시키는 보안 표면과 동시
  쓰기 clobber 위험이 커, OpenAI 권장대로(§2) **캐시**에만 보존한다. 캐시 축출
  (7일 미접근/용량) 시엔 시드 시크릿으로 떨어지므로, 그때는 재로그인이 필요하다.
- **되돌릴 조건:** 회전 관리가 계속 성가시면 CI 한정으로 구독 인증 대신
  `OPENAI_API_KEY`(회전 없음, 대신 토큰당 과금 — §2 재검토)로 바꾸는 선택지를
  그때 이 문서에 남긴다.
